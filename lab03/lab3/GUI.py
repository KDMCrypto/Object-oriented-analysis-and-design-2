...
class MainWindow(QMainWindow):
    ...
    def load_models(self):
        ...
        try:
            ...
            # Обновляем UI, чтобы текст лога появился сразу
            QApplication.processEvents() 
            
            from paddleocr import LayoutDetection
            self.layout_model = LayoutDetection(model_name="PP-DocLayout-L") # PP-DocLayoutV3 / PP-DocLayout-L
            
            open_h = OpenImageHandler()
            lay_h = LayoutHandler(self.layout_model)
            ocr_h = OCRHandler(model_name='qwen3.5:2b') 
            save_h = ImageSaveHandler()
            md_h = MarkdownHandler()

            open_h.set_next(lay_h).set_next(ocr_h).set_next(save_h).set_next(md_h)
            self.chain = open_h

            self.models_loaded = True
            self.btn_run.setEnabled(True)
            self.log("✅ Система готова. Модели успешно загружены.")
    ...
    def start_processing(self):
        ...
        try:
            self.log(f"--- ЗАПУСК ОБРАБОТКИ ---")
            self.btn_run.setEnabled(False)
            self.result_display.clear()
            self.tabs.setCurrentIndex(2) # Переключаемся на логи
            QApplication.processEvents()

            context = ImageContext(img_path, out_path)
            self.chain.handle_request(context)
            ...
            if hasattr(context, 'final_markdown'):
                ...
                self.result_display.setStyleSheet("background-color: #ffffff; color: #000000; font-size: 14px;")
                self.result_display.setMarkdown(context.final_markdown)
                self.tabs.setCurrentIndex(1) # Открываем вкладку "Результат"
